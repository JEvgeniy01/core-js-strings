...
function removeFirstOccurrences(str, value) {
  ...
  const returnedStr = str.slice(0, index) + str.slice(index + value.length);
  return returnedStr;
}
...
function formatTime(minutes, seconds) {
  // let formedMin = String(minutes);
  // let formedSec = String(seconds);
  // if (formedMin.length === 1) {
  //   formedMin = `0 + ${formedMin}`;
  // }
  // if (formedSec.length === 1) {
  //   formedSec = `0 + ${formedSec}`;
  // }
  // const result = `${formedMin}:${formedSec}`;
  // return result;
...
function isPalindrome(str) {
  const strWithOutSymbols = str.replace(/[\s.,!?]/g, '');
  const arrForLowerCase = strWithOutSymbols.split('');
  const arrNew = arrForLowerCase.map((item) => {
    return item.toLowerCase();
  });
  const initialString = arrNew.join('');
  const checkedString = arrNew.reverse().join('');
  if (initialString === checkedString) {
    return true;
  }
  return false;
}
...
function extractNameFromTemplate(value) {
  ...
  return value.slice(helloString.length, value.length - 1);
}
